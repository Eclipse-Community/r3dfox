...
--*/
{
    ...
    while (1) {
        ...
        if (OldValue.Value == 0) {
            ...
            WaitBlock.WakeEvent = CreateEventA ( NULL, FALSE, FALSE, NULL );
...
class MOZ_STATIC_CLASS Win32SRWLock final {
 public:
  ...
  void LockShared() { ExAcquirePushLockShared((EX_PUSH_LOCK *)&mLock); }

  void UnlockShared() { ExReleasePushLockShared((EX_PUSH_LOCK *)&mLock); }

  void LockExclusive() { ExAcquirePushLockExclusive((EX_PUSH_LOCK *)&mLock); }

  void UnlockExclusive() { ExReleasePushLockExclusive((EX_PUSH_LOCK *)&mLock); }
