...
--*/
{
    ...
    while (1) {
        ...
        if (OldValue.Value == 0) {
            ...
            if ( NULL == ( WaitBlock.WakeEvent = CreateEvent ( NULL, FALSE, FALSE, NULL ) ) )
                *((volatile int*)NULL) = 0x1337;
...
{
    ExInitializePushLock((PEX_PUSH_LOCK)m);
    return 0;
}
...
{
    ExAcquirePushLockExclusive((PEX_PUSH_LOCK)m);
    return 0;
}
static inline int pthread_mutex_unlock(pthread_mutex_t *m)
{
    ExReleasePushLockExclusive((PEX_PUSH_LOCK)m);
    return 0;
}
...
pthread_cond_init (pthread_cond_t *c, const void *a)
{
  ...
  if ( !(_c = (cond_t *)HeapAlloc (GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(*_c))) ) {
      return ENOMEM; 
  }
  _c->valid  = DEAD_COND;
  _c->busy = 0;
...
static int
pthread_cond_timedwait_impl (pthread_cond_t *c, pthread_mutex_t *external_mutex, const struct timespec *abstime, int rel)
{
  sCondWaitHelper ch;
#if 0
  DWORD dwr;
...
tryagain:
  ...
  ch.external_mutex = external_mutex;
  {
    ...
    r = pthread_mutex_unlock(external_mutex);
    if (!r)
      r = do_sema_b_wait (_c->sema_q, 0, t,&_c->waiters_q_lock_,&_c->value_q);
